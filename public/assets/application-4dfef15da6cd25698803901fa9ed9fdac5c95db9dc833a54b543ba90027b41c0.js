...
/* 표 높이 */
         ...
         if (options && $.isFunction(options.height)) {
            ...
            this.element.find(".wc-scrollable-grid").height(calendarHeight);
         }
      },
      ...
      _renderCalendar : function() {
         ...
         if (options.buttons) {
            ...
            //         </div>";
            calendarNavHtml = "<div class=\"wc-nav\">\
                <ul class = \"wc-nav-left\">\
                    <img src=\"images/hansung_logo.png\"  width ='30' height ='30'>\
                    </img>\
                </ul>\
                <ul class = \"wc-nav-center\">\
                    <h2 class=\"room_name\" width = '100' height = '30' text-align = \"right\"> </h2>\
                </ul>\
                <ul class = \"wc-nav-right\">\
                    <button class=\"wc-prev\">" + options.buttonText.lastWeek + "</button>\
                    <button class=\"wc-today\">" + options.buttonText.today + "</button>\
                    <button class=\"wc-next\">" + options.buttonText.nextWeek + "</button>\
                </ul>\
...
//빨간부분!!!!!
         ...
         //currentTime = self._dateFirstDayOfWeek(self._cloneDate(self.element.data("startDate"))).getTime();
         $weekDayColumns.each(function(i, val) {
            //var options = this.options;
            ...
            var pxTop = (self.options.date.getMinutes() + ((self.options.date.getHours() - 9) * 60)) * 6;
            
            var line = "<div id=\"line\" style =\"line-height: 28px; op: 360px; height: 3px; display: block; top:" + pxTop + "px; width: 100%; left: 0%; right: 0px;\">";
            ...
            if (self._isToday(currentDay)) {
               $(this).parent().addClass("wc-today"); // 이부분 
                 $(this).append(line);
      
               //$(this).css({"border", "solid 3px red"});
               //$(this).css({"border-top": "thick double #ff0000", "font-size": "200%"});
...
/**/
      ...
      _refreshEventDetails : function(calEvent, $calEvent) {
         ...
         $calEvent.find(".wc-title").html(calEvent.leaderField1 + '　'+ calEvent.leaderField2 + '<br>'  + calEvent.body);
   ...
   $.extend($.ui.weekCalendar, {
      ...
      defaults: {
         ...
         startParam : "start",
         endParam : "end",
         businessHours : {start: 9, end: 21, limitDisplay : false},
...
$(document).ready(function() {
   ...
   $calendar.weekCalendar({
      ...
      eventNew : function(calEvent, $event) {
         ...
         var leaderField1 = $dialogContent.find("input[name='leader_number']");
		 var leaderField2 = $dialogContent.find("input[name='leader_name']");
         var member1Field1 = $dialogContent.find("input[name='member_number1']");
		 var member1Field2 = $dialogContent.find("input[name='member_name1']");
         ...
         $dialogContent.dialog({
            ...
            buttons: {
               예약 : function() {
                  ...
                  calEvent.leaderField1 = leaderField1.val();
   				  calEvent.leaderField2 = leaderField2.val();
   				  calEvent.member1Field1 = member1Field1.val();
   				  calEvent.member1Field2 = member1Field2.val();
      ...
      eventClick : function(calEvent, $event) {
         ...
         var startField = $dialogContent.find("select[name='start']").val(calEvent.start);
         var endField = $dialogContent.find("select[name='end']").val(calEvent.end);
         var leaderField1 = $dialogContent.find("input[name='leader_number']").val(calEvent.leaderField1);
		 var leaderField2 = $dialogContent.find("input[name='leader_name']").val(calEvent.leaderField2);
         var member1Field1 = $dialogContent.find("input[name='member_number1']").val(calEvent.member1Field1);
		 var member1Field2 = $dialogContent.find("input[name='member_name1']").val(calEvent.member1Field2);
         var bodyField = $dialogContent.find("textarea[name='body']").val(calEvent.body);
         bodyField.val(calEvent.body);

         $dialogContent.dialog({
            modal: true,
            title: "예약수정 - " + calEvent.title,
            close: function() {
               $dialogContent.dialog("destroy");
               $dialogContent.hide();
               $('#calendar').weekCalendar("removeUnsavedEvents");
      ...
      return {
         events : [
            ...
            },
            {
             ...
             {
               "id":7,
               "start": new Date(year, month, day+2, 11),
